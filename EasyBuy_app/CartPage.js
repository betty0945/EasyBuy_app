...
const CartPage = () => {
 ...
 const handleUpdateQuantity = async (itemId, delta) => {
  ...
  try {
    // Add logging to see what we're looking for
    ...
    // If no item found, log and return
    if (!itemToUpdate) {
      console.log('No item found with ID:', itemId);
      return;
    }

    // Create updated items array with map instead of findIndex
    const updatedItems = items.map(item => {
      if (item.itemId === itemId) {
        console.log('Updating quantity for item:', item.name);
        return {
          ...item,
 ...
 const renderCartItems = () => {
  ...
  <View key={item.itemId || index} style={styles.cartItem}>
    ...
    <View style={styles.itemDetails}>
      ...
      <Text style={styles.itemPrice}>{`Price: $${(item.price || 0).toFixed(2)}`}</Text>
 ...
 return (
   <SafeAreaView style={styles.safeArea}>
     ...
     <View style={styles.searchBar}>
       <TextInput placeholder="Search your cart" style={styles.searchInput} />
     ...
     <View style={styles.checkoutContainer}>
       <Text style={styles.totalText}>{`Total: $${(total || 0).toFixed(2)}`}</Text>
